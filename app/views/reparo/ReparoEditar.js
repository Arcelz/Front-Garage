app.controller('ReparoEditar', function ($scope, $state, $q, $stateParams, DataService, MudarCor) {
    ...
    DataService.realizarGet('veiculos').then(function (data) {
        $scope.veiculosResultados = data.data;
        DataService.realizarGet('tipos-reparos').then(function (data2) {
            $scope.tiposResultados = data2.data;
            var clicked = true;
            var clickedVeiculo = true;
            DataService.realizarGet('reparos/' + id).then(function (response) {
                console.log(response)
                DataService.realizarGet('veiculos/' + response.data[0].fk_veiculo).then(function (data) {
                ...
                DataService.realizarGet('tipos-reparos/' + response.data[0].fk_tipos).then(function (data2) {
    ...
    $scope.salvar = function () {
        ...
        if ($scope.form.fkVeiculo != "" && $scope.form.fkTipo != "") {
            $scope.botao = true;
            DataService.realizarPut('reparos/' + id, $scope.form).then(function (response) {
